...
module Database.DSH.VSL.Opt.Properties.VectorType where
...
inferVectorTypeNullOp op =
  case op of
    Lit (ty, _, _)      -> Right $ VProp $ VTDataVec ty
...
inferVectorTypeUnOp s op =
  case op of
    ...
    Distinct -> VProp <$> unpack s
    MergeMap -> Right $ VProp $ VTNA
...
inferVectorTypeBinOp s1 s2 op =
  case op of
    ...
    UnboxDefault _ -> reqValVectors s1 s2 (\w1 w2 -> VPropPair (VTDataVec $ w1 + w2) VTNA) "UnboxSng"
    Materialize -> do
        VTDataVec w <- unpack s2
        return $ VPropPair (VTDataVec w) VTNA
    UpdateMap -> return $ VProp VTNA
