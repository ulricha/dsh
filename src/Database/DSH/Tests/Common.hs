...
makeProp :: (Q.QA a, Q.QA c, BackendVector b, VectorLang v)
         => (c -> c -> Bool)
         -> (Q.Q a -> Q.Q c)
         -> (a -> c)
         -> a
...
makePropEq :: (Eq c, Q.QA a, Q.QA c, BackendVector b, VectorLang v)
           => (Q.Q a -> Q.Q c)
           -> (a -> c)
           -> a
           -> DSHProperty v b
...
makePropDouble :: (Q.QA a, BackendVector b, VectorLang v)
               => (Q.Q a -> Q.Q Double)
               -> (a -> Double)
               -> a
               -> DSHProperty v b
...
testPropertyConn :: (Show a, Arbitrary a)
                 => BackendCodeGen v b
                 -> BackendConn b
                 -> TestName
