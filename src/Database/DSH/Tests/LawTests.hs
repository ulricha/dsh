...
tests_laws :: (BackendVector b, VectorLang v) => DSHTestTree v b
...
prop_reverse_identity :: (BackendVector b, VectorLang v) => [Integer] -> DSHProperty v b
prop_reverse_identity xs codeGen conn = monadicIO $ do
    let q = Q.reverse $ Q.reverse (Q.toQ xs)
    res <- run $ runQ codeGen conn q
    assert $ res == xs

prop_reverse_sort :: (BackendVector b, VectorLang v) => OrderedList Integer -> DSHProperty v b
prop_reverse_sort (Ordered xs) codeGen conn = monadicIO $ do
    let q = Q.sortWith id $ Q.reverse (Q.toQ xs)
    res <- run $ runQ codeGen conn q
    assert $ res == xs

prop_reverse_sort_tuple :: (BackendVector b, VectorLang v) => OrderedList (Integer, Integer) -> DSHProperty v b
