...
and e = let t = typeOf e
         in if ListT PBoolT P.== t
            then AppE1 PBoolT (Agg L.And) e
            else tyErrShow "and" [t]

or :: Expr -> Expr
or e = let t = typeOf e
         in if ListT PBoolT P.== t
            then AppE1 PBoolT (Agg L.Or) e
            else tyErr "or"
...
sum e = let (ListT t) = typeOf e
         in if isNum t
                then AppE1 t (Agg L.Sum) e
                else tyErr "sum"


avg :: Expr -> Expr
avg e = case typeOf e of
            ListT PDoubleT  -> AppE1 PDoubleT (Agg L.Avg) e
            ListT PDecimalT -> AppE1 PDecimalT (Agg L.Avg) e
            _              -> tyErr "avg"

minimum :: Expr -> Expr
minimum e = let (ListT t) = typeOf e
             in if isNum t
                 then AppE1 t (Agg L.Minimum) e
                 else tyErr "minimum"

maximum :: Expr -> Expr
maximum e = let (ListT t) = typeOf e
             in if isNum t
                 then AppE1 t (Agg L.Maximum) e
                 else tyErr "maximum"
