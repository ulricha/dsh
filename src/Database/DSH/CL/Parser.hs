...
prim1 =     try (kw "singleton" *> pure Singleton)
        ...
        <|> try (kw "length" *> pure (Agg Length))
        <|> try (kw "concat" *> pure Concat)
        <|> try (kw "null" *> pure Null)
        <|> try (kw "sum" *> pure (Agg Sum))
        <|> try (kw "avg" *> pure Only)
        <|> try (kw "minimum" *> pure (Agg Minimum))
        <|> try (kw "maximum" *> pure (Agg Maximum))
        <|> try (kw "reverse" *> pure Reverse)
        <|> try (kw "and" *> pure (Agg And))
        <|> try (kw "or" *> pure (Agg Or))
        <|> try (kw "nub" *> pure Nub)
