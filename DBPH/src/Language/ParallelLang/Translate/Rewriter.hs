...
rewriteTree r e = recurse e
  where
    ...
    rewriteAST' (CloApp t e1 e2) = (CloApp t) <$> recurse e1 <*> recurse e2
    rewriteAST' (CloLApp t e1 e2) = (CloLApp t) <$> recurse e1 <*> recurse e2
    rewriteAST' (Clo t l fvs x f fl) = (Clo t l fvs x) <$> recurse f <*> recurse fl
    rewriteAST' (AClo t fvs x f fl) = (AClo t fvs x) <$> recurse f <*> recurse fl
